        ...
        if (SPEED_THRESH < source->speed_kph) {
            if (source->lon_deg != INVALID_LON && source->lat_deg != INVALID_LAT) {
                if (prev_lon != INVALID_LON && prev_lat != INVALID_LAT) {
                    ...
                    update_calced_count++;
#endif
                    double current_distance = calc_distance(prev_lon, source->lon_deg, prev_lat, source->lat_deg);
                    if (lock) {
                        lock();
                    }
...
#ifdef CONFIG_CALC_DISTANCE_DEBUG
                    printf("%.4f,%.4f,%d,%d,%d",current_distance, accumurated_distance, update_called_count, update_calced_count, get_called_count);
#endif
